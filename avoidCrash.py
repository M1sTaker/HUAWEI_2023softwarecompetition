...
def avoid_wall(robot, line_speed, angle_speed, pre_speed, map):
...
def crash_detect(robot_list, map, crash_detect_distance=2, ignore_lowspeed=3 ):
    ...
    for i in range(len(robot_list)):  # 遍历整个机器人列表,robot为待检测
        for j in range(i + 1, len(robot_list)):
            ...
            crash_threshold = 2 * np.arcsin(0.45 / robot_12_dis) * 1.4  # 计算临界碰撞角度 [1.4]
    ...
    for crash in crash_list:
        ...
        if abs(robot_2['angle_speed']) > 2:
            ...
            if robot_2['rotate_state'] != 0:
                ...
                if map == 3:  # [3,4]|[3]+avoidwall_speed[1]
