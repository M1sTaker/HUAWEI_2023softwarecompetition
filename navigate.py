...
def move_to_xy(robot, des_x, des_y):
    ...
    # if (np.abs(robot_face_angle) + np.abs(xy_robot_vec_angle) <= np.pi/2 or np.abs(robot_face_angle) + np.abs(xy_robot_vec_angle) >= 2*np.pi - np.pi/2):
