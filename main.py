...
if __name__ == '__main__':
    ...
    while True:
        ...
        print("得分:"+int(parts[1]), file=sys.stderr)
        ...
        for i in range(4):
            line = sys.stdin.readline().strip().split(' ')
            robot_list.append({'work_bench_id': int(line[0]), 'item_tpye': int(line[1]), 'time_cost': float(line[2]),
        ...
        for robot_id in range(4):
            line_speed, angle_speed = move_to_xy(robot_list[robot_id], work_bench[robot_id]['x'],
                                                 work_bench[robot_id]['y'])
